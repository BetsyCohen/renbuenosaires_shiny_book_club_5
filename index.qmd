...
El capítulo se recorren 4 posibles formas de feedback usando el ayudín
...
-   Todas tienen \[tres argumentos obligatorios\].{blue-text}
    ...
    poner: \[color\].{purple-text} y \[icon\].{purple-text}
:::
::::
::::::
...
server]{.fragment .highlight-purple fragment-index="1"}. Por ej. podemos
poner como requisito que *no muestre el resultado erróneo*
...
A veces tenemos más de una condición que queremos *freezar* y hay que
esperar a que el usuario termine de hacer algo. Por ej. en este caso
necesitamos que termine de elegir el idioma (`selectInput()`) y ponga su
nombre (`textInput()`) para que se ejecute el **greetings** en el
`paste0()`
...
![](assets/cat-theme/head-02.svg){.absolute bottom=50 right=370 width="150" height="150"}
![](assets/cat-theme/head-03.svg){.absolute bottom=50 right=220 width="150" height="150"}
![](assets/cat-theme/head-04.svg){.absolute bottom=50 right=70 width="150" height="150"}
...
El siguiente código muestra un ejemplo sencillo donde [no queremos]{.brown-text}
registrar ni calcular la raíz cuadrada de valores negativos en x.
...
Las notificaciones las vamos a ir viendo apiladas en la parte inferior
derecha de la app.

Tipos de notificación:

-   **transitoria**: desaparece automáticamente después de un período de
    tiempo fijo.
-   **del proceso**: cuando se inicia un proceso y eliminarla cuando el
    proceso finaliza.
-   **progresivas**: actualizar una sola notificación con
...
-   La forma más sencilla de usarlo showNotification() es llamarlo con
    un solo argumento: el mensaje que queres mostrar.
-   De forma predeterminada, el mensaje desaparecerá después de 5
    segundos y podemos anular esto con la opción \[ duration\] o el user
    puede cancelarlo haciendo click en la cruceita de cerrar
-   Si desea que la notificación sea más visible, se puede configurar el
    argumento type de modo que tome el css del theme para "message",
    "warning" o "error"
...
Para simular una notificación progresiva en lugar de mostrar
notificaciones distintas (una debajo de la otra como si fuera un
sanguchito) podemos jugar con el parámetro **id** de
`showNotification()` de forma que va a parecer como si fuera
actualizando el texto de la misma notificación.
...
:::: fragment
::: {.speech .pink}
![](assets/cat-theme/profile-01.svg) Proveer distracción: con imágenes,
textos (con un dato curioso por ej.), sonidos.
:::
::::

:::: fragment
::: {.speech .purple}
![](assets/cat-theme/profile-02.svg) Dar una explicación de por qué
estas esperando
:::
::::
...
:::: fragment
::: {.speech .blue}
![](assets/cat-theme/profile-03.svg) Dar certeza respecto de cuánto
tiempo llevará.
:::
::::
...
::: {.spacer style="height:70%;"}
En el libro vemos dos soluciones a las barras de progreso una nativa de
shiny y otra con el paquete `waiter{}`
...
waitress y hostess La acción se define en 4 pasos
...
**waiter()** es ideal para hacer spinners permite hacer una barra un
poco más chulas a nivel visual y por ejemplo editar en más detalle el
html o agregar un gif (por ej si queres animar el logo institucional)

Por ej acá usamos un spin_flower() para que muestre una flor mientras
carga el gráfico
...
Por deafult waiter muestra un spinner (y podemos elegir entre más de 100
modelos!) pero también podemos hacer barras usando la funcion
waitress.Para usarlo llamamops a `useWaitress()` en el UI y definimos el
waitress con `Waitress$new`. Dentro de `Waitress$new` nos permite setear
a que elemento le vamos a aplicar la barra (`selector`), así como un
`theme` `max` y `min` de la barra.
...
`shinycssloaders` Simplemente agregamos shinycssloaders::withSpinner()
para envolver el output que queremos que *"spinee"* automaticamente y
asereje!
...
creamos un nuevo modo de interacción: no se puede interactuar con la app
principal hasta que no se haya procesado el diálogo. Lo definimos
**fuera del server** y después lo usamos de manera dinámica como eventos
condicionales **dentro** del mismo
